    ...
    (let ((hit (cache-sim-query-cache sim address)))
      (if hit
        ...
        (let* ((mem-cell (aref (cache-sim-mem-cells sim) address))
               (cache-block (cache-sim-lru-in-line sim address))
          ...
          (setf (caddr cache-block)
                (loop for x from 0 below (expt 2 (cache-sim-block-bits sim))
                      collect (aref (cache-sim-mem-cells sim)
                                    (+ line-address x))))
          (values 'miss mem-cell cache-block cache-block-backup))))))
